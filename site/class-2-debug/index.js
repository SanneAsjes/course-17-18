...
/* Scales and axes. */
// in de nieuwe versie van d3 is het niet scale.linear maar scaleLinear.
var x = d3.scaleLinear().range([0, width]);
var y = d3.scaleLinear().range([height, 0]);
var color = d3.scaleOrdinal().range(['#fe2f2f', '#feca2f', '#96fe2f']);

// blijkbaar is dit de nieuwe manier van aangeven ofzo....
var xAxis = d3.select(".axis")
            .call(d3.axisBottom(x));
var yAxis = d3.select(".axis")
            .call(d3.axisLeft(y));

/* Size SVG. */ //heb effe entertje omhoog geflikkerd
var svg = d3.select('svg')
  .append('svg')
  .attr('viewBox',[0,
    0,
    margin.l + width + margin.r,
    margin.t + height + margin.b].join(' ')) // toegevoegd
  .append('g')
...
function onload(data) {
  x.domain(d3.extent(data, sepalWidth)).nice(); //extend moet met een t.
